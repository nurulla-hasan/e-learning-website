...
import { useMakePaymentMutation } from "@/redux/feature/payment/paymentApi";
...
const Checkout = () => {
  ...
  const checkoutId = data?.data[0]?.id || "";
  const checkoutOrders: CheckoutOrder[] = Array.isArray(data?.data)
    ? (data!.data as CheckoutOrder[])
    : [];
  const subtotal = checkoutOrders.reduce(
    (sum: number, order: CheckoutOrder) => sum + (order.totalAmount || 0),
    0
  );
  ...
  const handleMakePayment = async () => {
    try {
      ...
      const response = await makePayment({ checkoutId }).unwrap();
                                ...
                                {typeof ci.discountPrice === "number" ? (
                                  <div className="flex flex-col items-end">
                                    <span className="text-sm text-muted-foreground line-through">
                                      zł {ci.price.toFixed(2)}
                                    </span>
                                    <span className="text-base font-semibold text-primary">
                                      zł {ci.discountPrice.toFixed(2)}
                                    </span>
                                  </div>
                                ) : (
                                  <span className="text-base font-semibold text-primary">
